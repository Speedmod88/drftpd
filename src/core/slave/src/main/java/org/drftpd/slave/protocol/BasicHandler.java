...
public class BasicHandler extends AbstractHandler {
    ...
    public AsyncResponse handleConnect(AsyncCommandArgument ac) {
        ...
        logger.info("!! HANDLE CONNECT. encrypted=" + encrypted + ". Address=" + address + ". Port=" + port + ". useSSLClientHandshake=" + useSSLClientHandshake + ". BindIP=" + getSlaveObject().getBindIP());
        ...
        return new AsyncResponseTransfer(ac.getIndex(), new ConnectInfo(port, t.getTransferIndex(), t.getTransferStatus(), null));
    }
    ...
    public AsyncResponse handleListen(AsyncCommandArgument ac) {
        ...
        boolean useLanIP = data[2].equals("true");
        PassiveConnection c;

        try {
            c = new PassiveConnection(encrypted ? getSlaveObject().getSSLContext() : null,
                    getSlaveObject().getPortRange(), useSSLClientMode, useLanIP ? getSlaveObject().getSlaveLanIP() : getSlaveObject().getBindIP());
    ...
    public AsyncResponse handleRemerge(AsyncCommandArgument ac) {
        ...
        try {
            ...
            if (partialRemerge) {
                ...
                remergeDecision = "Instant online: " + (instantOnline ? "enabled" : "disabled")
                        + ", Partial remerge: enabled. skipping all files last modified before " + cutoffDate + ".";
            ...
            getSlaveObject().awaitRemergeResponsesFlushed();
