...
public class Root {
    ...
    {
	getAllInodes(inodeTree, cancelled, null);
    }

    public void getAllInodes(Map<String, DirectoryContent> inodeTree, BooleanSupplier cancelled, Boolean oldestOnDuplicateFile)
                throws IllegalArgumentException, IOException
    {
        ...
        var walker = new FileTreeWalker(inodeTree, cancelled, oldestOnDuplicateFile);
        walker.Walk(getPath());
    }
    ...
    private class FileTreeWalker extends SimpleFileVisitor<Path> {
        ...
        private final Boolean oldestOnDuplicateFile;

        public FileTreeWalker(Map<String, DirectoryContent> inodeTree, BooleanSupplier cancelled) {
		this(inodeTree, cancelled, null);
        }


        public FileTreeWalker(Map<String, DirectoryContent> inodeTree, BooleanSupplier cancelled, Boolean oldestOnDuplicateFile) {
            _inodeTree = inodeTree;
            _cancelled = cancelled;
            this.oldestOnDuplicateFile = oldestOnDuplicateFile;
        }
        ...
        private void AddFile(Path path, BasicFileAttributes attrs) throws IOException {
			...
			if (oldestOnDuplicateFile != null && oldestOnDuplicateFile.booleanValue()) {
				if (attrs.lastModifiedTime().compareTo(inodeAttributes.lastModifiedTime()) > 0) {
					//file being added is more recent that file already added, keep old, log only and skip.
                            dirContents.inodes.put(path.getFileName().toString(), attrs);
                            _inodeTree.put(rootRelativeParentPath, dirContents);
                            logger.warn("!!DUPLICATE FILE DETECTED ON SLAVE ROOTS. LATEST ROOT ANALYZED HAVE NEWER COPY. File=" + path.getFileName().toString() + ". NewerDate=" + attrs.lastModifiedTime() + ". OlderDate=" + inodeAttributes.lastModifiedTime() + ".");
				}
				else {
					//file being added is older then the file already added, replace and log
                            dirContents.inodes.put(path.getFileName().toString(), attrs);
                            _inodeTree.put(rootRelativeParentPath, dirContents);
                            logger.warn("!!DUPLICATE FILE DETECTED ON SLAVE ROOTS. LATEST ROOT ANALYZED HAVE OLDER COPY. File=" + path.getFileName().toString() + ". NewerDate=" + inodeAttributes.lastModifiedTime() + ". OlderDate=" + attrs.lastModifiedTime() + ".");
				}
			}
			else {
